...
\textcolor{red}{This setting can be generalized to incorporate a general weighting scheme, as \cite{chiouMultivariateFunctionalPrincipal2014} or \cite{happMultivariateFunctionalPrincipal2018a}. We consider the function $\inH{\cdot}{\cdot}_w~:~\HH \times \HH \rightarrow \RR$,
...
\textcolor{red}{If we observe the set $\mathcal{X}$, the ideal estimators of the mean and covariance function would be}
...
\end{equation}
\textcolor{red}{with the $\varepsilon_{n, \mathsf{m}}$ being independent copies of a centered error random vector $\varepsilon \in \RR^{P}$ with finite variance.}

\textcolor{red}{Let $\widehat{X}_n$ be a suitable nonparametric estimator of $X_n$ applied to the pairs $(Y_{n, \mathsf{m}}, \mathrm{T}_{n, \mathsf{m}})$, such as smoothing splines or local polynomials.}
\begin{equation}
    \textcolor{red}{\widehat{\mu}(\pointt) = \sum_{n = 1}^N \pi_n \widehat{X}_n(\pointt), \quad \pointt \in \TT{}}
\end{equation}
\textcolor{red}{Concerning the estimation of the covariance, we distingush the diagonal from the non-diagonal points.}
\begin{equation}
    \textcolor{red}{\widehat{C}(\points, \pointt) =  \sum_{n = 1}^N \pi_n \{\widehat{X}_n(\points) - \widehat{\mu}(\points)\}\{\widehat{X}_n(\pointt) - \widehat{\mu}(\pointt)\}^\top}
\end{equation}
\textcolor{red}{The estimation of the covariance function is given by}
\begin{equation}
...
\end{equation}
\textcolor{red}{The variance function $C_{p, p}(s_p, t_p)$ induces a singularity when estimating the covariance function $C_{p, p}(\cdot, \cdot)$ (\cite{yaoFunctionalDataAnalysis2005,zhangSparseDenseFunctional2016}).}
\begin{equation}
    \textcolor{red}{\widehat{C}_{p, p}(s_p, s_p) = \sum_{n = 1}^N \pi_n\{\hatXnp{p}(s_p) - \hatmup{p}(s_p)\}^2 - \sigma_p^2, \quad s_p \in \TT{p}.}
\end{equation}

\textcolor{red}{Following \cite{benkoCommonFunctionalPrincipal2009,grithFunctionalPrincipalComponent2018},}
\begin{equation}
...
\end{equation}
\textcolor{red}{Similarly, a correction for the diagonal of the Gram matrix is given by}
\begin{equation}
    \textcolor{red}{\widehat{\mathbf{M}}_{nn} = \pi_n\sum_{p = 1}^P \int_{\TT{p}}\{\hatXnp{p}(t_p) - \widehat{\mu}^{(p)}(t_p)\}^2 \dd t_p - \sigma_p^2, \quad n = 1, \dots, N.}
\end{equation}

\textcolor{red}{For the estimation of the variance, \cite{hallVarianceEstimationNonparametric1990,hallAsymptoticallyOptimalDifferenceBased1990}}
