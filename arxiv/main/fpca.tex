...
\begin{equation}\label{eq:kl_multi_trunc_comp}
    X_{\lceil K_p \rceil}^{(p)}(t_p) = \mup{p}(t_p) + \sum_{k = 1}^{K_p} \mathfrak{c}_k \phi_k^{(p)}(t_p), \quad t_p \in \TT{p}, \quad K_p \geq 1, \quad 1 \leq p \leq P,
\end{equation}
...
\begin{remark}
\add{This can be extended to general weighting schemes by defining a slightly different version of the Gram matrix where each entries is given by
\begin{equation}
   M_{nn^\prime} = \pi_n\pi_{n^\prime} \inH{X_n - \mu}{X_{n^\prime} - \mu}, \quad n, n^\prime = 1, \dots, N. 
\end{equation}}
\end{remark}
