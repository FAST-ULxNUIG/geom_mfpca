...
Concerning the standardization of the data, there are two main proposals in the literature. \cite{happMultivariateFunctionalPrincipal2018a} propose \textcolor{red}{to consider the weights}
\begin{equation}
w_p = \left(\int_{\TT{p}} \Var X^{(p)}(t_p) \dd t_p\right)^{-1}.
\end{equation}
This standardization is coherent with the derivation of the total inertia of the observation space using the usual distance in $\HH$. \cite{chiouMultivariateFunctionalPrincipal2014} and \textcolor{red}{\cite{jacquesModelbasedClusteringMultivariate2014a} propose the weighting function}
\begin{equation}
w_p(t_p) = \left(\Var X^{(p)}(t_p)\right)^{\textcolor{red}{-1}}, \quad t_p \in \TT{p}.
\end{equation}
...
Considering the duality diagram and the total inertia of the clouds with respect to the distance $d_\Gamma$, \textcolor{red}{we propose to use the weights}
\begin{equation}
    w_p = \left(\int_{\TT{p}} \normH{C_{p \cdot}(t_p, \cdot)}^2 \dd t_p\right)^{\textcolor{red}{-1}}.
\end{equation}
The total inertia of $\CP$ (and $\CN$) will thus be equal to the number of components.
\textcolor{red}{In every case, we may consider the rescaled elements $\widetilde{X}^{(p)} = w_p^{1/2}\{X^{(p)} - \mup{p}\}$ in place of the elements $X^{(p)}$.}
