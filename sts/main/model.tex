...
We consider the function $\inH{\cdot}{\cdot} : \HH \times \HH \rightarrow \RR$, such that, for $f, g \in \HH$,
\begin{equation}\label{eq:innerprodH}
    \inH{f}{g} \coloneqq \sum_{p=1}^{P}\int_{\TT{p}} \fp(t_p)\gp(t_p) \dd t_p.
\end{equation}
$\HH$ is a Hilbert space with respect to the inner product $\inH{\cdot}{\cdot}$\citep{happMultivariateFunctionalPrincipal2018a}. We denote by $\normH{\cdot}$, the norm induced by $\inH{\cdot}{\cdot}$. Let $\mu : \TT{} \rightarrow \HH$ denote the mean function of the process $X$, $\mu(\pointt) \coloneqq \EE(X(\pointt)),\,\pointt \in \TT{}$. Let $C$ denote the $P \times P$ matrix-valued covariance function which, for $\points, \pointt \in \TT{}$, is defined as, for all $\points, \pointt \in \TT{}$,
\begin{equation}\label{eq:covariance_function}
    C(\points, \pointt) \coloneqq \EE\left(\{X(\points) - \mu(\points)\}\{X(\pointt) - \mu(\pointt)\}^{\top}\right).
\end{equation}
More precisely, for $1 \leq p, q \leq P$, the $(p, q)$th entry of the matrix $C(\points, \pointt)$ is the covariance function between the $p$th and the $q$th features of the process $X$ such that for all $s_p \in \TT{p}, t_q \in \TT{q}$,
\begin{align*}\label{eq:covariance_function_components}
    C_{p, q}(s_p, t_q) &\coloneqq \\
     &\mkern-30mu\EE\left(\{\Xp{p}(s_p) - \mup{p}(s_p)\}\{\Xp{q}(t_q) - \mup{q}(t_q)\}\right).
\end{align*}
Let $\Gamma : \HH \rightarrow \HH$ denote the covariance operator of $X$, defined as an integral operator with kernel $C$. That is, for $f \in \HH$ and $\pointt \in \TT{}$, the $p$th feature of $\Gamma f(\pointt)$ is given by, for all $t_p \in \TT{p}$,
\begin{equation}\label{eq:covariance_operator_components}
    (\Gamma f)^{(p)}(t_p) \coloneqq \inH{C_{p, \cdot}(t_p, \cdot)}{f(\cdot)} = \inH{C_{\cdot, p}(\cdot, t_p)}{f(\cdot)}.
\end{equation}

Let us consider a set of $N$ multivariate curves $\XX = \{X_1, \ldots, X_n, \ldots, X_N\}$ generated as a random sample of the $P$-dimensional stochastic process $X$ with continuous trajectories. Unless otherwise stated, the data are assumed to be observed without error. The data can be viewed as a table with $N$ rows and $P$ columns where each entry is a curve, potentially on a multidimensional domain (see Figure~\ref{fig:data_matrix}). Each row of this matrix represents an observation; while each column represents a functional feature. At the intersection of row $n$ and column $p$, we thus have $\Xnp$ which is the curve that concerns the (functional) feature $p$ for the individual $n$.
...
For a given $p \in \{1, \dots, P\}$, the mean curve of the $p$th feature along the $N$ observations is denoted by $\mup{p}$. This quantity can be computed as, for $p \in \{1, \dots, P\}$ and for all $t_p \in \TT{p}$,
\begin{equation*}\label{eq:mu_estimation}
    \mup{p}(t_p) = \sum_{n = 1}^N \pi_n\Xnp(t_p).
\end{equation*}
The cross-covariance function of the $p$th and $q$th features along the $N$ observations can be computed as, for $p = 1, \dots, P$ and $q = 1, \dots, P$, for all $s_p \in \TT{p}, t_q \in \TT{q}$,
\begin{align}\label{eq:cov_estimation}
    C_{p, q}(s_p, t_q) &= \sum_{n = 1}^N \pi_n\Xnp(s_p)\Xnq(t_q) \\
    &\qquad - \mup{p}(s_p)\mup{q}(t_q).
\end{align}
For the set $\mathcal{X}$, the inner-product matrix, also called the Gram matrix, $\mathbf{M}$ is defined as a matrix of size $N \times N$ with entries $\mathbf{M}_{nn^\prime}$ defined for $n, n^\prime = 1, \dots, N$, as
\begin{equation}\label{eq:gram_mat}
    \mathbf{M}_{nn^\prime} \coloneqq \sqrt{\pi_n \pi_{n^{\prime}}}\inH{X_n - \mu}{X_{n^\prime} - \mu}.
\end{equation}
...
\end{equation}
Similarly, the covariance function of the $p$th and $q$th features is given, for all $s_p \in \TT{p}, t_q \in \TT{q}$, by:
\begin{align*}
    C_{p,q}(s_p, t_q) &= \\
    &\mkern-60mu\sum_{k = 1}^{K_p} \sum_{l = 1}^{K_q} \left(\sum_{n = 1}^N \pi_n c^{(p)}_{nk}c^{(q)}_{nl} - \overline{c}_k^{(p)}\overline{c}_l^{(q)}\right)\psi_k^{(p)}(s_p)\psi_l^{(q)}(t_q).
\end{align*}
...
\begin{equation}
    \mu(\pointt) = \Psi(\pointt)^\top \mathbf{C}^\top\Pi
\end{equation}
and
\begin{equation}
C(\points, \pointt) = \Psi(\points)^\top \mathbf{C}^\top \left(\text{diag}\{
...
\end{equation}
$\mathrm{I}_{\!N}$ is the identity matrix of size $N$ and $\mathbf{1}_{\!N}$ is a vector of $1$ of length $N$.
